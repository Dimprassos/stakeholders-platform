        ...
        <nav aria-label="Primary" className="flex flex-wrap items-center gap-4 text-sm sm:gap-6">
          {NAV.map((item) => (
            <NavLink
              key={item.href}
              href={item.href}
              className="nav-link text-zinc-600 transition-colors hover:text-foreground aria-[current=page]:font-medium aria-[current=page]:text-brand-accent dark:text-zinc-400"
            >
              {item.label}
            </NavLink>
          ))}
